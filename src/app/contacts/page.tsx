...
// Screen 7: Contact List — filterable table with persona, warmth, company link

'use client';

import React, { useState, useEffect, useCallback } from 'react';
import { createClient } from '@/lib/supabase-client';
import type { ContactWarmth, ContactPersona, CompanyStatus } from '@/lib/supabase-types';
import { WARMTH_LABELS, WARMTH_COLORS, PERSONA_LABELS, STATUS_LABELS } from '@/lib/supabase-types';

const supabase = createClient();

function WarmthBadge({ warmth }: { warmth: ContactWarmth }) {
  ...
  return (
    <div>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table className="data-table">
            <thead>
              <tr>
                <th onClick={() => handleSort('last_name')} style={{ cursor: 'pointer', userSelect: 'none' }}>
                  Name{sortArrow('last_name')}
                </th>
                <th>Company</th>
                <th>Title</th>
                <th onClick={() => handleSort('persona')} style={{ cursor: 'pointer', userSelect: 'none' }}>
                  Persona{sortArrow('persona')}
                </th>
                <th onClick={() => handleSort('warmth')} style={{ cursor: 'pointer', userSelect: 'none' }}>
                  Warmth{sortArrow('warmth')}
                </th>
                <th>Email</th>
                <th>Email Status</th>
                <th>Location</th>
                  ...
                  <td>
                    {contact.persona ? (
                      <span className="badge badge-blue" style={{ fontSize: 'var(--text-xs)' }}>
                        {PERSONA_LABELS[contact.persona as ContactPersona] || contact.persona}
                      </span>
                    ) : '—'}
                  </td>
                  <td><WarmthBadge warmth={contact.warmth} /></td>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'var(--space-5)' }}>
        ...
        <button className="btn btn-secondary btn-sm" disabled={contacts.length < PAGE_SIZE} onClick={() => setPage(p => p + 1)}>
          Next →
        </button>
      </div>
    </div>
